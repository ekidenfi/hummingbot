"""
Activity Simulator Controller

Generates trading activity with zero net position by placing round-trip trades.
Simple flow: flat -> open -> close -> flat (flip side) -> repeat
...
class ActivitySimulatorConfig(ControllerConfigBase):
    ...
    use_orderbook_prices: bool = Field(
        default=True, json_schema_extra={"is_updatable": True}
    )
    max_orderbook_depth: int = Field(
        default=10, json_schema_extra={"is_updatable": True}
    )
    spread_bps: Decimal = Field(
        default=Decimal("10"), json_schema_extra={"is_updatable": True}
    )

    min_order_size_quote: Decimal = Field(
        default=Decimal("10"), json_schema_extra={"is_updatable": True}
    )
    max_order_size_quote: Decimal = Field(
        default=Decimal("50"), json_schema_extra={"is_updatable": True}
    )

    order_interval_seconds: float = Field(
        default=2.0, json_schema_extra={"is_updatable": True}
    )
    stale_order_seconds: float = Field(
        default=5.0, json_schema_extra={"is_updatable": True}
    )
...
class ActivitySimulator(ControllerBase):
    ...
    def get_exchange_position(self) -> Decimal:
        try:
            ...
            logger.warning(f"[SIM] Error getting exchange position: {e}")
        return Decimal("0")

    def generate_random_size_quote(self) -> Decimal:
        min_size = float(self.config.min_order_size_quote)
        max_size = float(self.config.max_order_size_quote)
    ...
    def get_crossing_price(self, side: TradeType) -> Optional[Decimal]:
        try:
            ...
            entries = list(
                order_book.ask_entries()
                if side == TradeType.BUY
                else order_book.bid_entries()
            )
            if not entries:
                return None
    ...
    def calculate_order_price(self, side: TradeType) -> Decimal:
        ...
        return (
            mid_price * (Decimal("1") + spread)
            if side == TradeType.BUY
            else mid_price * (Decimal("1") - spread)
        )
    ...
    def check_stale_executors(self) -> List[ExecutorAction]:
        ...
        for executor in self.active_executors():
            if current_time - executor.timestamp >= self.config.stale_order_seconds:
                ...
                logger.info(f"[SIM] Cancelling stale executor {executor.id}")
        return actions

    def should_place_order(self) -> bool:
        current_time = self.market_data_provider.time()
        return (
            current_time - self._last_order_time >= self.config.order_interval_seconds
    ...
    def determine_executor_actions(self) -> List[ExecutorAction]:
        ...
        if not mid_price or mid_price <= 0:
            logger.warning(f"[SIM] Invalid mid_price: {mid_price}")
            return []

        position = self.get_exchange_position()
        logger.info(
            f"[SIM] Exchange position={position}, last_open_side={self._last_open_side}"
        )

        if position == Decimal("0"):
            if self._last_open_side is None:
                side = random.choice([TradeType.BUY, TradeType.SELL])
                logger.info(f"[SIM] First order, random side: {side.name}")
            else:
                side = (
                    TradeType.SELL
                    if self._last_open_side == TradeType.BUY
                    else TradeType.BUY
                )
                logger.info(
                    f"[SIM] Flipping: last was {self._last_open_side.name}, now {side.name}"
                )
            ...
            logger.info(f"[SIM] -> OPEN {side.name} amount={amount:.4f} @ {price:.6f}")
            ...
            logger.info(
                f"[SIM] -> CLOSE {side.name} amount={amount:.4f} @ {price:.6f} (position={position})"
            )
    ...
    def to_format_status(self) -> List[str]:
        ...
        state = (
            "FLAT"
            if position == Decimal("0")
            else ("LONG" if position > 0 else "SHORT")
        )
        last_side = self._last_open_side.name if self._last_open_side else "NONE"
        next_side = (
            "SELL"
            if self._last_open_side == TradeType.BUY
            else "BUY"
            if self._last_open_side
            else "RANDOM"
        )
        ...
        return [
            ...
            f"│ Last: {last_side:<6} Next: {next_side:<6} │ Trips: {self._completed_roundtrips:<4} │ Replaced: {self._replaced_orders:<4} │ Active: {len(self.active_executors()):<2}│",
            "└" + "─" * 80 + "┘",
        ]
