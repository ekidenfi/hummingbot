...
class EkidenPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
            ...
            ekiden_perpetual_api_secret: str = None,
            use_vault: bool = False,
            ekiden_perpetual_api_key: str = None,
            trading_pairs: Optional[List[str]] = None,
            trading_required: bool = True,
            domain: str = CONSTANTS.DOMAIN,
    ):
        self.ekiden_perpetual_api_key = ekiden_perpetual_api_key
        self.ekiden_perpetual_secret_key = ekiden_perpetual_api_secret
    ...
    @property
    def name(self) -> str:
        # Note: domain here refers to the entire exchange name. i.e. ekiden_perpetual or ekiden_perpetual_testnet
        return self._domain

    @property
    def authenticator(self) -> Optional[EkidenPerpetualAuth]:
        if self._trading_required:
            return EkidenPerpetualAuth(self.ekiden_perpetual_api_key, self.ekiden_perpetual_secret_key,
                                       self._use_vault)
        return None
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        if cancel_result.get("status") == "err" or "error" in cancel_result["response"]["data"]["statuses"][0]:
            self.logger().debug(f"The order {order_id} does not exist on Ekiden Perpetuals. "
