...
from hummingbot.connector.derivative.ekiden_perpetual.ekiden_perpetual_utils import (
    EkidenPerpetualConfigMap,
    EkidenPerpetualTestnetConfigMap,
    validate_bool,
)


class EkidenPerpetualUtilsTests(TestCase):
    pass
    ...
    def test_cls_validate_bool_succeed(self):
        ...
        validations = [EkidenPerpetualConfigMap.validate_bool(value) for value in valid_values]
        for validation in validations:
    ...
    def test_cls_validate_bool_fails(self):
        ...
        with self.assertRaises(ValueError) as exception_context:
            EkidenPerpetualConfigMap.validate_bool(wrong_value)
    ...
    def test_cls_testnet_validate_bool_succeed(self):
        ...
        validations = [EkidenPerpetualTestnetConfigMap.validate_bool(value) for value in valid_values]
        for validation in validations:
            self.assertTrue(validation)
    ...
    def test_cls_testnet_validate_bool_fails(self):
        ...
        valid_values = ('true', 'yes', 'y', 'false', 'no', 'n')
        with self.assertRaises(ValueError) as exception_context:
            EkidenPerpetualTestnetConfigMap.validate_bool(wrong_value)
        self.assertEqual(str(exception_context.exception), f"Invalid value, please choose value from {valid_values}")
